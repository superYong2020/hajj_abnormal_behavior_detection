...
const.HEIGHT = 256
const.WIDTH = 256
const.FLOWNET_CHECKPOINT = '../history/checkpoints/pretrains/flownet-SD.ckpt-0'
...
const.SAVE_DIR = '{dataset}_{time}'.format(dataset=const.DATASET,time = time.strftime('%y%m%d%H%M'))
...
if args.snapshot_dir:
    ...
    if os.path.exists(args.snapshot_dir + '.meta') or os.path.exists(args.snapshot_dir + '.data-00000-of-00001') or \
        ...
        const.SNAPSHOT_DIR = get_dir(os.path.join('../history/checkpoints', const.SAVE_DIR + '_' + args.snapshot_dir))
        # const.SNAPSHOT_DIR = get_dir(os.path.join('../history/checkpoints', 'hajj_2009010913/model.ckpt-100'))
else:
    const.SNAPSHOT_DIR = get_dir(os.path.join('../history/checkpoints', const.SAVE_DIR))
    # const.SNAPSHOT_DIR = get_dir(os.path.join('../history/checkpoints', 'hajj_2009010913/model.ckpt-100'))

if args.summary_dir:
    const.SUMMARY_DIR = get_dir(os.path.join('../history/summary', const.SAVE_DIR + '_' + args.summary_dir))
else:
    const.SUMMARY_DIR = get_dir(os.path.join('../history/summary', const.SAVE_DIR))

if args.psnr_dir:
    const.PSNR_DIR = get_dir(os.path.join('../history/psnrs', const.SAVE_DIR + '_' + args.psnr_dir))
else:
    const.PSNR_DIR = get_dir(os.path.join('../history/psnrs', const.SAVE_DIR))

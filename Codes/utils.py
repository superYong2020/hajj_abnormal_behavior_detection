...
class DataLoader(object):
    ...
    def setup(self):
        """读入数据集，并存到videos字典中 videos每个子文件夹包括三个字段，path，frame和length"""
        videos = glob.glob(os.path.join(self.dir, '*'))
        for video in sorted(videos):
            video_name = video.split('/')[-1]
            self.videos[video_name] = {}#video_name是代表01,02，相当于为每个文件创建一个词典，内容包括路径，所有的帧以及包括多少帧
    ...
    def get_video_clips(self, video, start, end):
        ...
        return np.concatenate(batch, axis=2)#合成256*256*n的矩阵

...
with tf.name_scope('dataset'):
    ...
    train_inputs = train_videos_clips_tensor[..., 0:num_his*3]#要输入的前4张图片
    train_gt = train_videos_clips_tensor[..., -3:]#要预测的真实的图片
...
if adversarial:
    ...
    adv_loss = tf.reduce_mean(tf.square(fake_outputs - 1) / 2)#生成器
    dis_loss = tf.reduce_mean(tf.square(real_outputs - 1) / 2) + tf.reduce_mean(tf.square(fake_outputs) / 2)#判别器
else:

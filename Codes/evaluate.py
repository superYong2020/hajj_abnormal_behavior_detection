...
class RecordResult(object):
    ...
    def __lt__(self, other):#重定义小于
        return self.auc < other.auc

    def __gt__(self, other):#重定义大于
        return self.auc > other.auc
...
class GroundTruthLoader(object):
    ...
    NAME_MAT_MAPPING = {
        ...
        UMN : os.path.join(DATA_DIR, 'umn03/umn.mat'),
        HAJJ: os.path.join(DATA_DIR, '/home/dataset_2020/UMN_all/hajj/hajj.mat'),
    ...
    NAME_FRAMES_MAPPING = {
        ...
        UMN: os.path.join(DATA_DIR, 'umn03/testing/frames'),
        HAJJ: os.path.join(DATA_DIR, '/home/dataset_2020/UMN_all/hajj/testing/frames'),
    ...
    def __load_ucsd_avenue_subway_gt(self, dataset):
        ...
        mat_file = self.mapping[dataset]#代表测试文件中异常的开始与结束
        ...
        return gt#相当于为每个异常视频帧打上1的标签，正常帧为0标签
    @staticmethod
...
def compute_auc(loss_file):
    ...
    for sub_loss_file in loss_file_list:
        ...
        for i in range(num_videos):
            ...
            scores = np.concatenate((scores, distance[DECIDABLE_IDX:]), axis=0)#psnr作为预测值
            labels = np.concatenate((labels, gt[i][DECIDABLE_IDX:]), axis=0)
        ...
        if os.path.isdir(loss_file):
            print(results)
    print('##### optimal result and model = {}'.format(optimal_results))
    file_handle=open('08_31.txt',mode='a')
    file_handle.write('##### optimal result and model = {} \n'.format(optimal_results))
    file_handle.close()
